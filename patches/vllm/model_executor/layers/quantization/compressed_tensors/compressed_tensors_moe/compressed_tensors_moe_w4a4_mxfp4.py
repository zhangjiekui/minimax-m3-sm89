...
class CompressedTensorsW4A4Mxfp4MoEMethod(CompressedTensorsMoEMethod):
    def __init__(self, moe):
        ...
        # PATCH (minimax-m3): the cutlass MXFP4 kernel drops the SwiGLU-OAI clamp
        # (run_cutlass_moe_mxfp4 -> apply_moe_activation without clamp_limit), so
        # honor VLLM_MXFP4_USE_MARLIN=1 to force the Marlin path, which threads
        # gemm1_clamp_limit correctly via get_fused_moe_quant_config below.
        import os as _os
        _force_marlin = _os.environ.get("VLLM_MXFP4_USE_MARLIN", "").lower() in (
            "1", "true", "yes",
        )
        self.use_cutlass_mxfp4 = (
            CutlassExpertsMxfp4._supports_current_device() and not _force_marlin
        )
    ...
    def get_fused_moe_quant_config(
        ...
        # PATCH (minimax-m3): forward the SwiGLU-OAI clamp params. Without
        # swiglu_limit the SWIGLUOAI_UNINTERLEAVE kernel asserts on a missing
        # gemm1_clamp_limit. make_mxfp4_moe_quant_config maps swiglu_limit ->
        # gemm1_clamp_limit per backend; the W4A4 builder needs it set directly.
        if self.use_cutlass_mxfp4:
